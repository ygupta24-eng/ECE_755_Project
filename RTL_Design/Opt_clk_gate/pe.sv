module pe #(
    ...
    // ── Incoming mux — gated by move_en ──────────────────────────
    // Optimization 2: when move_en=0 no data is transferring so
    // hold incoming at '0 to prevent unselected bus toggles from
    // propagating into move_reg on the next clock edge.
    ...
    // Optimization 1: combinational operand isolation (no added latency)

    assign act_val = en ? move_reg[DATA_W-1:0] : '0;
    assign w_val   = en ? w_reg                : '0;
