...
logic [5:0] image_mem [0:511][0:511][0:3];

int x_prev, y_prev;

initial begin
    fd = $fopen("psum_results_rose.txt", "w");
    if (!fd) $display("Error: Could not open file for writing.");
    $readmemh("rose_square_6bit_4ch.txt", image_mem);
    ...
    for(int x =0; x<509; x=x+2) begin
        for(int y = 0; y<509; y=y+2) begin
            if(!(x==0 && y==0)) begin
