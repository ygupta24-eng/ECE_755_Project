module zig_zag_4x4 #(
    ...
    // FIX: removed always_latch using ~clk (not synthesizable by Yosys, creates
    // a clock-gated latch that OpenLane cannot handle). The en signal is already
    // a registered FSM output — connect it directly to each PE's clock-enable.
        ...
        for (genvar r = 0; r < ROWS; r++) begin : g_pe_row
            for (genvar c = 0; c < COLS; c++) begin : g_pe_col
                pe #(
                    ...
                    .clk            (clk),
                    .rst_n          (rst_n),
                    .en             (en),          // FIX: wired directly; PE uses as clock-enable
                    .move_en        (move_en),
    ...
    assign psum_out0 = act_to_left[1][0];  // PE10
    assign psum_out1 = act_to_left[3][0];  // PE30
